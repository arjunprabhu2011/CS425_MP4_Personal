 ...
 */
pub fn check_list_for_failure(machine_number: usize, membership_list: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>, suspicion_active: &Arc<Mutex<bool>>, leader_node: &Arc<Mutex<LeaderListener>>, leader_name_global: &Arc<Mutex<String>>) {
    loop {
        if let Ok(mut locked_membership_list) = membership_list.lock() {
            let current_time = SystemTime::now();

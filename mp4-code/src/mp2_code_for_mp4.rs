...
pub const INTRODUCER_NAME: &str = "fa23-cs425-4601.cs.illinois.edu:80";  

pub const RANDOM_SELECTION_COUNT: usize = 4;
pub const PORT_8000: usize = 8000;

pub const SUSPICION_TIMEOUT: u64 = 4;
pub const FAILURE_TIMEOUT: u64 = 5;  
pub const CLEANUP_TIMEOUT: u64 = 4;
 ...
 */
fn run_cli(membership_list: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>, machine_number: usize, suspicion_active: &Arc<Mutex<bool>>, arc_drop_rate: &Arc<Mutex<u32>>) {
    loop {
 ...
 * membership_list - membership list 
 */
pub fn list_mem(membership_list: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>) {
 ...
 * membership_list - membership list 
 */
pub fn list_self(machine_number: usize, membership_list: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>) {
    if let Ok(locked_membership_list) = membership_list.lock() {
 ...
 * suspicion_active - is suspicion active
 */
pub fn send_tcp_message(machine_number: usize, message: &str, suspicion_active: &Arc<Mutex<bool>>) {
    let domains: [&str; 10] = [
 ...
 */
pub fn read_suspicion_and_drop_rate_changes(mut stream: TcpStream, suspicion_active: &Arc<Mutex<bool>>, arc_drop_rate: &Arc<Mutex<u32>>) -> io::Result<()> {
    let mut buf = [0; 1024];
    loop {
 ...
 * arc_drop_rate: the drop rate of packets  
 */
pub fn listen_for_suspicion_and_drop_rate_changes(suspicion_active: &Arc<Mutex<bool>>, machine_number: usize, arc_drop_rate: &Arc<Mutex<u32>>) -> io::Result<()> {
    let domains: [&str; 10] = [
        "fa23-cs425-4601.cs.illinois.edu:80",
        "fa23-cs425-4602.cs.illinois.edu:80",
        "fa23-cs425-4603.cs.illinois.edu:80",
 ...
 */
pub fn membership_listener(machine_num: usize, membership_list: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>, udp_socket: &Arc<Mutex<UdpSocket>>, listener_drop_rate: &Arc<Mutex<u32>>) {
    loop {
        let mut buf = [0; 1024];
        ...
        if let Some(received_list) = received_list {
            merge_membership_lists(&received_list, membership_list, machine_num);
        }
 ...
 * udp_socket: socket to send from 
 */
pub fn membership_sender(membership_list: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>, udp_socket: &Arc<Mutex<UdpSocket>>) {
    loop {
        if let Ok(locked_membership_list) = membership_list.lock() {
 ...
 */
pub fn merge_membership_lists(membership_list_one: &HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>, membership_list_two: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>, machine_num: usize) {
 ...
 */
pub fn check_list_for_failure(machine_number: usize, membership_list: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>, suspicion_active: &Arc<Mutex<bool>>, leader_node: &Arc<Mutex<LeaderListener>>, leader_name_global: &Arc<Mutex<String>>, leader_maplejuice: &Arc<Mutex<LeaderMapleJuice>>, leader_name_maplejuice: &Arc<Mutex<String>>) {
    loop {
        if let Ok(mut locked_membership_list) = membership_list.lock() {
            let current_time = SystemTime::now();
            ...
            let mut leader_nodes_to_send_list: Vec::<Arc<String>> = Vec::new();

            for key in nodes_to_delete.iter() {
                let node_id_arc = Arc::new(key.0.clone());
                ...
                leader_nodes_to_send_list.push(Arc::new(key.0.clone()));                
            }

            drop(locked_membership_list);

            for x in leader_nodes_to_send_list.iter() {
                if let Ok(locked_leader) = leader_node.lock() {
                    if let Ok(leader_name) = leader_name_global.lock() {
                        if *locked_leader.machine_domain_name == *leader_name {
                            locked_leader.send_all_node_failure_requests(x.clone());
                        }
                    }
                }
            }

            for y in leader_nodes_to_send_list.iter() {
                if let Ok(mut locked_leader_maplejuice) = leader_maplejuice.lock() {
                    if let Ok(leader_name_mj) = leader_name_maplejuice.lock() {
                        if *locked_leader_maplejuice.machine_domain_name == *leader_name_mj {
                            locked_leader_maplejuice.send_task_replication_requests(y.clone());
                        }
                    }
                }
            }
...
* mem_list - the membership list
*/
pub fn handle_node_leave(machine_number: usize, membership_list: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>) {
    if let Ok(mut locked_membership_list) = membership_list.lock() {
        let port = PORT_8000 + machine_number;
...
*/
pub fn node_join(machine_number: usize, mem_list: &Arc<Mutex<HashMap<(String, usize, SystemTime), (u64, u64, NodeStatus)>>>, udp_socket: &Arc<Mutex<UdpSocket>>) {
